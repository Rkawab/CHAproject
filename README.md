...
- **固定費管理** - 家賃、光熱費(水道のみ隔月)などの月間固定支出を管理
- **サブスク管理** - Netflix・Spotifyなど継続課金サービスを登録・管理。開始月・終了月を設定して月ごとに自動集計
...
- **サブスク設定**：サブスクを登録・管理（ユーザー情報ページから遷移）
...
│
├── fixedcosts/               # 固定費管理アプリ
│   ├── models.py             # FixedCost モデル・Subscription モデル
│   ├── views.py              # 固定費 CRUD・サブスク CRUD
│   └── ...
│
...
5. **サブスク** を登録（ユーザー情報ページ → 「サブスク設定変更」から。Netflix・Spotifyなど、開始月・終了月を設定）

...
def fixedcosts_list(request, year=None, month=None):
    ...
    future_limit = next_month_date > today_next

    # 前月・次月のデータ存在確認
    prev_exists = FixedCost.objects.filter(year=prev_year, month=prev_month).exists()
    next_exists = (
        FixedCost.objects.filter(year=next_year, month=next_month).exists()
        if not future_limit
        else False
    )
    ...
    if fc_first and fc_last:
        ...
        available_months = build_available_months(
    ...
    if fixed_cost:
        if fixed_cost.water is not None:
            ...
            # 入力なし → 前月データから取得
            prev_month_fc = FixedCost.objects.filter(
        ...
        for label, value, note in items:
            if value and value > 0 and label != "家賃":
                chart_data.append({"cost_item__name": label, "total": value})

    return render(
        request,
        "fixedcosts/list.html",
        {
...
def fixedcosts_delete(request, year, month):
    ...
    return render(
        request,
        "fixedcosts/delete_confirm.html",
        {
            "fixed_cost": fixed_cost,

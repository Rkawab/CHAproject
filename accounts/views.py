...
def regist(request):
    ...
    )  # request.POST or None：フォームのデータが送信されている場合 → request.POST をフォームに渡す
    if (
    ...
    return render(
        ...
        context={  # バリデーションに失敗した場合の処理(ifがFalse)
...
def user_login(request):
    ...
    if login_form.is_valid():
        ...
        )  # 認証。 DjangoのAUTH_USER_MODEL に登録されていて、かつパスワードが一致する場合、該当のユーザーオブジェクトを返す
        if user:
            login(
                request, user
            )  # ログイン状態に。request.userの値を該当のユーザーに更新する

...
class UserActivateTokenManager(models.Manager):
    ...
    def activate_user_by_token(
        self, token
    ):  # やっていることはUserのデータベースの1つを書き換えているだけ(is_activate：0 → 1)
    ...
    def create_or_update_token(self, user):
        ...
        user_token, created = (
            self.update_or_create(  # self.update_or_createはこのクラスにないので、親クラスのManagerのインスタンスを使う
                user=user,
                defaults={
                    "token": token,
...
class UserActivateToken(models.Model):
    ...
    objects: UserActivateTokenManager = (
        UserActivateTokenManager()
    )  # objects は UserActivateTokenManager 型ですよ、という型ヒント
...
)  # デコレーター (Userモデルが 保存（post_save）されたあとに自動的に呼ばれる処理)
def publish_token(

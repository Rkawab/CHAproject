...
class RegistForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        if password != confirm_password:
            ...
            )  # add_error(指定したフィールド, エラーメッセージ) (djangoにあるインスタンス)
            # → パスワードが書かれているフィールドに、一致しないメッセージを載せる指示
        try:
            validate_password(
                password, self.instance
            )  # validate_password(パスワード, ユーザー) (djangoにあるインスタンス)
    ...
    def save(self, commit=False):
        user = super().save(
            commit=False
        )  # このメソッドが完了するまではまだデータベースへの保存はしない
...
class UserActivateForm(forms.Form):
    ...
    )  # HiddenInputはフォームに表示されない隠しフィールドになっている
...
class PasswordChangeForm(forms.ModelForm):
    ...
    def save(
        self, commit=False
    ):  # commitをTrueにして実行しないと保存できないようになる
